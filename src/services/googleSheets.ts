...
console.log('Environment variable:', import.meta.env.VITE_GOOGLE_SHEETS_API_URL ? 'SET' : 'NOT SET')
console.log('URL value:', GOOGLE_APPS_SCRIPT_URL)
console.log('=====================================')

if (!GOOGLE_APPS_SCRIPT_URL) {
  console.error('URL da API do Google Sheets não configurada. Defina VITE_GOOGLE_SHEETS_API_URL no arquivo .env')
  alert('Erro: URL da API não configurada. Configure no arquivo .env')
} else if (GOOGLE_APPS_SCRIPT_URL.includes('YOUR_DEPLOYMENT_ID') || GOOGLE_APPS_SCRIPT_URL.includes('YOUR_')) {
  console.error('URL da API do Google Sheets contém placeholder. Atualize o arquivo .env com o ID correto do seu deployment')
  alert('Erro: A URL contém um placeholder. Atualize o arquivo .env com o ID correto do Google Apps Script')
}
...
export const googleSheetsService = {
  ...
  async listarCalendario(): Promise<Operacao[]> {
    try {
      const url = GOOGLE_APPS_SCRIPT_URL
      console.log('Fetching calendar from:', url)
      const response = await fetch(url, {
        method: 'POST',
        body: new URLSearchParams({ action: 'listCalendario' })
      })
      const data: SheetResponse = await response.json()
  ...
  async criarOperacao(operacao: Omit<Operacao, 'id' | 'status' | 'createdAt' | 'updatedAt' | 'createdBy' | 'updatedBy'>): Promise<boolean> {
    try {
  ...
  async atualizarCalendario(): Promise<boolean> {
    try {
      const url = GOOGLE_APPS_SCRIPT_URL
      const response = await fetch(url, {
        method: 'POST',
        body: new URLSearchParams({ action: 'atualizarCalendario' })
      })
      const data = await response.json()
      return data.status === 'success'
    } catch (error) {
  ...
  async criarCalendario(): Promise<boolean> {
    try {
      const url = GOOGLE_APPS_SCRIPT_URL
      const response = await fetch(url, {
        method: 'POST',
        body: new URLSearchParams({ action: 'criarCalendario' })
      })
      const data = await response.json()
      return data.status === 'success'
    } catch (error) {
  ...
  mapearOperacao(data: Record<string, unknown>): Operacao {
    ...
    return {
      ...
      organizacaoPolicialMilitar: String(getVal(['organizacaoPolicialMilitar', 'ORGANIZAÇÃO POLICIAL MILITAR', 'organização', 'BPM'])),
      municipio: String(getVal(['municipio', 'MUNICÍPIO', 'MUNICÍPIO/DISTRITO'])),
      empregoPoliciciamento: String(getVal(['empregoPoliciciamento', 'EMPREGO DE POLICIAMENTO', 'emprego'])),
      nomeOperacaoApoio: String(getVal(['nomeOperacaoApoio', 'NOME DA OPERAÇÃO/APOIO', 'nome operação'])),
      area: String(getVal(['area', 'ÁREA'])),
      orgaoDemandante: String(getVal(['orgaoDemandante', 'ÓRGÃO DEMANDANTE', 'orgão', 'orgão demandante'])),
      ...
      recursoFinanceiroEmpregado: Number(getVal(['recursoFinanceiroEmpregado', 'RECURSOS FINANCEIROS EMPREGADOS', 'RECURSO FINANCEIRO EMPREGADO', 'RECURSO EMPREGADO', 'recurso'], 0)),
      valorGasto: Number(getVal(['valorGasto', 'VALOR GASTO', 'gasto'], 0)),
      codOperacaoSiseg: String(getVal(['codOperacaoSiseg', 'CÓD. OPERAÇÃO SISEG', 'cod_siseg', 'CÓD. SISEG'])),
      ...
      updatedAt: String(getVal(['updatedAt', 'ATUALIZADO EM', 'updatedAt'], new Date().toISOString())),
